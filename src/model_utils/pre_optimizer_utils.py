...
def get_total_steps(trainer, hparams):
    ...
    if trainer.max_epochs > 0:
        ...
        total_steps = (len(train_loader.dataset) * trainer.max_epochs // tb_size) // ab_size
    else:
...
def add_optimizer_args(parent_args):
    parser = parent_args.add_argument_group('Basic Module')
    parser.add_argument('--random_seed', default=72, type=int)
    parser.add_argument('--learning_rate', default=5e-3, type=float)
    parser.add_argument('--min_learning_rate', default=5e-4, type=float)
    parser.add_argument('--lr_decay_steps', default=0, type=int)
    parser.add_argument('--lr_decay_ratio', default=1.0, type=float)
    parser.add_argument('--warmup_steps', default=0, type=int) ## 3
...
def configure_optimizers(pl_model: LightningModule, model_params=None):
    ...
    if isinstance(pl_model.trainer.strategy, DeepSpeedStrategy):
        ...
        # optimizer = AdamW(optimizer_grouped_params, lr=pl_model.hparams.learning_rate, betas=(pl_model.hparams.adam_beta1, pl_model.hparams.adam_beta2), eps=pl_model.hparams.adam_epsilon)
        optimizer = torch.optim.AdamW(optimizer_grouped_params, lr=pl_model.hparams.learning_rate,
                          betas=(pl_model.hparams.adam_beta1, pl_model.hparams.adam_beta2),
                          eps=pl_model.hparams.adam_epsilon)
    # Configure learning rate scheduler.
    total_steps = pl_model.hparams.lr_decay_ratio * pl_model.total_steps if pl_model.hparams.lr_decay_steps == 0 else pl_model.hparams.lr_decay_steps
    warmup_steps = pl_model.hparams.warmup_ratio * pl_model.total_steps if pl_model.hparams.warmup_steps == 0 else pl_model.hparams.warmup_steps

    if pl_model.hparams.scheduler_type == "inverse_sqrt":
        scheduler = inverse_square_root_schedule(optimizer=optimizer, num_warmup_steps=warmup_steps, lr_min=pl_model.hparams.warmup_min_lr, lr_max=pl_model.hparams.warmup_max_lr)
    else:
        scheduler = get_scheduler(name=pl_model.hparams.scheduler_type, optimizer=optimizer, num_warmup_steps=warmup_steps, num_training_steps=total_steps, lr_end=pl_model.hparams.min_learning_rate)
